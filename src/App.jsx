...
function LangProvider({ children }) {
  const [lang, setLang] = useState(() => {
    try { return localStorage.getItem("va-lang") || "id"; } catch { return "id"; }
  });
...
const css = `
  ...
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  ...
  .btn-primary:hover {
    ...
    box-shadow: 0 8px 28px rgba(27,60,53,0.2);
...
// ─── Logo Image ──────────────────────────────────────────────────────────
// Place your logo file as "logo.png" in the public/ folder of your Vite project.
// To use a different filename, update the LOGO_SRC constant below.
...
// ─── WhatsApp Config ─────────────────────────────────────────────────────
// Number is read from VITE_WHATSAPP_NUMBER environment variable.
// In Vite, env vars must start with VITE_ to be exposed to the frontend.
// Add VITE_WHATSAPP_NUMBER=6281234567890 in your .env or Vercel dashboard.
const WHATSAPP_NUMBER = import.meta.env.VITE_WHATSAPP_NUMBER || "";
console.log('wa: ' + WHATSAPP_NUMBER);

function getWhatsAppUrl(lang, t) {
  const msg = encodeURIComponent(t("whatsapp_message"));
  ...
  return (
    <a
      ...
      style={{
        ...
        gap: hovered ? 10 : 0,
        padding: hovered ? "0 22px 0 18px" : 0,
        ...
        <Reveal delay={i * 70}>
              ...
              >
                <div style={{ width: "100%", height: 180, borderRadius: 10, marginBottom: 22, background: theme.surfaceAlt, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <span style={{ fontSize: 13, color: theme.textMuted, fontWeight: 500 }}>{item.image}</span>
                </div>
          ...
          <Reveal>
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                {/* WhatsApp link */}
                <a href={getWhatsAppUrl(lang, t)} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 14, textDecoration: "none" }}>
                  ...
                  <span style={{ fontSize: 14, color: theme.text, transition: "color 0.3s" }}
                    onMouseEnter={e => e.target.style.color = "#25D366"} onMouseLeave={e => e.target.style.color = theme.text}>
                    {t("contact_whatsapp")}
                  </span>
                </a>
              </div>
